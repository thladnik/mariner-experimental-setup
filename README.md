## MARINER visual stimulation setup

This is the repository for MARINER - the **M**ultispectral **A**rena for **R**eal-time **I**nteractive **N**euronal and **E**thological **R**esearch. 

![MARINER rendering](https://github.com/thladnik/mariner-experimental-setup/blob/main/stimsetup_no_breadboard_no_chrolis_120mm.png)

### Overview

[MARINER light path schematic](https://github.com/thladnik/mariner-experimental-setup/blob/main/MARINER_light_path_schematic_drawing.pdf)  (incl. dimensions for possible configuration)

[Bill of materials](https://github.com/thladnik/mariner-experimental-setup/blob/main/bill_of_materials.csv)

[Printable components](https://github.com/thladnik/mariner-experimental-setup/tree/main/printed_parts) (Mesh and design files)

### Components

#### Projector
![MARINER projector](https://github.com/thladnik/mariner-experimental-setup/blob/main/components/MARINER_highlight_projector.png)

#### Center assembly
![MARINER center assembly](https://github.com/thladnik/mariner-experimental-setup/blob/main/components/MARINER_highlight_center.png)

#### Radial arms
![MARINER radial arms](https://github.com/thladnik/mariner-experimental-setup/blob/main/components/MARINER_highlight_side.png)

#### (optional) Behavioral camera
![MARINER camera](https://github.com/thladnik/mariner-experimental-setup/blob/main/components/MARINER_highlight_camera.png)
